...
- 🔔 **"Your partner is doing this now" Reminders:**
  - A local notification fires the moment a plan starts, worded by whose plan it is — *"Partnerin şu an: Spor salonu"*, *"Şu an ikinizin planı: Sinema"*, or your own.
  - Reminders are rebuilt automatically whenever plans change (locally or via sync); completed and past plans are skipped.
- ✅ **Completed / Not Completed Plans:**
  - Every plan carries a completion state, toggled from a round checkbox in the day, week and month schedules or from the plan editor.
  - Completed plans are struck through, drop out of the today surfaces, and the partner gets a *"… planını tamamladı ✅"* push.
- ⏳ **Passed Countdowns Auto-Hide:**
  - A *"kala"* counter whose date has gone by no longer clutters the counter strip. A **Geçmişi Göster** toggle brings them back so they can still be edited or deleted.
- 📝 **Multiple Notes per Day:**
  - A day now holds any number of notes, each with its own identity, edited and deleted individually and synced per note.
- 🕛 **Full-Day Timeline (00:00 → 23:59):**
  - The daily view covers every hour of the day instead of starting at 08:00.
- 📱 **Today Plan Home Screen Widgets:**
  - Native **Android App Widget** and **iOS WidgetKit** widget listing what is still planned for today.
  - Plans that already finished during the day are hidden, so the widget always answers *"what is left today"*.
  - An in-app **Today's Plan** card mirrors the same view at the top of the calendar.
- ⌚ **Native Time Pickers:**
  - Start/end time fields open the platform's own time picker instead of free-text entry.
...
### Home Screen Widgets
- **Android:** `modules/home-widget` — a local Expo module shipping an `AppWidgetProvider` that reads the app's SQLite database directly, so the widget stays correct even when the app has not run.
- **iOS:** `targets/today-widget` — a WidgetKit extension generated by [`@bacons/apple-targets`](https://github.com/EvanBacon/expo-apple-targets); the app publishes a JSON snapshot into the shared App Group (`group.com.ercode.microphonecheck`) and reloads the timelines.
